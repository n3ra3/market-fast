...
# ── Монитор статуса продаж (по /items, без Steam/куки) ───────────────────────
...
def _active_count(items: list[dict[str, Any]]) -> int:
    """Сколько лотов реально на продаже (status==1)."""
    n = 0
    for it in items:
        try:
            if int(float(it.get("status"))) == 1:
                n += 1
...
async def sales_monitor_loop() -> None:
    """Монитор статуса продаж БЕЗ Steam/куки. Периодически смотрит /items и, если
    число активных лотов упало до нуля (аккаунт ушёл в офлайн / лоты сняты), шлёт
    ОДНО уведомление в Telegram. Восстановление -> одно уведомление. Без спама.
    Продажи включаешь на маркете вручную."""
    if not config.SALES_MONITOR_ENABLED:
        logger.info("Sales monitor disabled (SALES_MONITOR_ENABLED=0)")
        return
    logger.info("Sales monitor: активные лоты /items каждые {}s",
                int(config.SALES_CHECK_INTERVAL_SEC))
    await asyncio.sleep(config.SALES_CHECK_INTERVAL_SEC)  # дать боту подняться
    seen_positive = False   # видели ли когда-нибудь активные лоты
    alerted_off = False     # уже слали алерт про «пропали»
    zero_streak = 0
    while True:
        try:
            items = await get_my_items()
            if items is None:
                logger.debug("sales monitor: /items недоступен — пропуск (состояние не трогаем)")
            else:
                active = _active_count(items)
                logger.debug("sales monitor: активных лотов {}", active)
                if active > 0:
                    seen_positive = True
                    zero_streak = 0
                    if alerted_off:
                        await _sale_alert(f"🟢 Лоты снова на продаже: {active}.")
                        alerted_off = False
                else:
                    zero_streak += 1
                    if seen_positive and not alerted_off and zero_streak >= config.SALES_ZERO_STREAK:
                        await _sale_alert(
                            "🔴 <b>Лоты пропали с продажи</b> — 0 активных.\n"
                            "Возможно, аккаунт ушёл в офлайн или лоты сняты. "
                            "Проверь и включи продажи на маркете.")
                        alerted_off = True
        except Exception:
            logger.exception("sales monitor iteration failed")
        await asyncio.sleep(config.SALES_CHECK_INTERVAL_SEC)
